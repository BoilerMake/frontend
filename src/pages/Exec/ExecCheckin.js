...
class ExecCheckin extends Component {
    ...
    render () {
        let a = this.props.exec.user_list.map(user=><CheckinCard key= {user.id} user={user} checkInClicked={this.checkInClicked}/>);
        let {name, hashid} = this.state;
        return (
            <div>
                <Header as='h1' dividing>Hacker Check In</Header>
                    ...
                    <Grid.Row>
                        ...
                        <Grid.Column>
                            <Form>
                                ...
                                <Form.Input placeholder='Shortcode' name='hashid' value={hashid} onChange={this.handleChange} />
                                <Form.Button type='submit' onClick={this.handleSubmit}>Submit</Form.Button>
                            </Form>
                        </Grid.Column>
                    </Grid.Row>
                </Grid>
                {this.props.exec.user_list_loading ? <div>Data Loading!! <Icon loading name='spinner' size='large'/></div> : null}
                {this.props.exec.user_list.length === 0 && !this.props.exec.user_list_loading ? <div> no results :(</div> : null}
...
export default connect(mapStateToProps, mapDispatchToProps)(ExecCheckin);
